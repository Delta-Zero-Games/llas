...
impl JitterBuffer {
    ...
    fn adapt_delay(&mut self, sequence: u32) {
        if sequence > self.last_sequence {
            ...
            if jitter > 0 {
                self.current_delay = (self.current_delay + jitter)
                    .min(self.max_delay);
            } else {
...
impl AudioNetwork {
    pub async fn new(bind_addr: &str, turn_config: TurnConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let turn_socket = Self::setup_turn_connection(&turn_config, socket.try_clone().await?).await?;
    ...
    async fn setup_turn_connection(
        ...
        let server_addr = config.url.parse()?;
        ...
        request.write_u16::<BigEndian>(20)?; // HMAC-SHA1 is 20 bytes
        request.write_all(&hmac)?;

        // Update message length
        let message_length = (request.len() - 20) as u16; // Exclude header
            ...
            loop {
                match socket.recv_from(&mut buffer).await {
                    Ok((size, addr)) => {
                        ...
                        if let Some(monitor) = quality_monitors.get_mut(&addr) {
...
fn process_turn_response(response: &[u8]) -> Result<Option<SocketAddr>, Box<dyn std::error::Error>> {
    ...
    while pos + 4 <= response.len() {
        ...
        if attr_type == XOR_MAPPED_ADDRESS {
            ...
            let port = u16::from_be_bytes([response[pos + 6], response[pos + 7]]) ^ (STUN_MAGIC_COOKIE >> 16) as u16;
...
impl QualityMonitor {
    ...
    fn update(&mut self, sequence: u32, received_time: Instant) {
        ...
        if let Some(last_latency) = self.latency_samples.get(self.latency_samples.len() - 2) {
    ...
    fn get_stats(&self) -> NetworkStats {
        ...
        let quality = match (avg_latency.as_millis(), packet_loss) {
            (l, p) if l < 50 && p < 0.01 => ConnectionQuality::Excellent,
            (l, p) if l < 100 && p < 0.02 => ConnectionQuality::Good,
            (l, p) if l < 150 && p < 0.05 => ConnectionQuality::Fair,
            (l, p) if l < 200 && p < 0.10 => ConnectionQuality::Poor,
